...
RSpec.feature "Officer Permissions" do
  context "when the officer is not an admin" do
    scenario "they cannot create a draft for someone already in the system" do
      sign_in_officer
    ...
    scenario "cannot draft a plan for a person who is not in the system", :js do
      officer = create(:officer)
      sign_in_officer(officer)

      visit people_path
      find(".menu-icon").trigger("click")
      expect(page).not_to have_link t("menu.new_person")
    end

    scenario "they cannot view drafts", :js do
      sign_in_officer create(:officer)
  ...
  context "when the officer is an admin" do
    ...
    scenario "they can kick a submission back down to draft status" do
      ...
      expect(page).
        to have_content t("submissions.destroy.success", name: plan.person.name)
      expect(current_path).to eq draft_path(plan)
      expect(page).to have_link t("drafts.show.edit")
    end

    scenario "they cannot approve a response plan" do
      officer = create(:officer, :admin)
      sign_in_officer(officer)
      ...
      expect(page).not_to have_link t("submissions.show.approve")
    end
  end

  context "when the officer is a owner" do
