...
RSpec.describe SafetyConcern, type: :model do
  describe "validations" do
    ...
    it "must be marked threat or physical if it's p1 or p2" do
      ...
      assault = build(:safety_concern, category: :assaultive_public, physical_or_threat: :physical)
      other = build(:safety_concern, category: :assaultive_law, physical_or_threat: :verbal)
      none = build(:safety_concern, category: :assaultive_public, physical_or_threat: nil)
    ...
    it "cannot be marked threat or physical if it's p3 or p4" do
      threat = build(:safety_concern, category: :weapon, physical_or_threat: :threat)
      assault = build(:safety_concern, category: :chemical, physical_or_threat: :physical)
      other = build(:safety_concern, category: :weapon, physical_or_threat: :verbal)
      none = build(:safety_concern, category: :chemical, physical_or_threat: nil)
  ...
  describe "#physical?" do
    it "returns true if the concern has a physical component" do
      concern = build(:safety_concern, category: :assaultive_law, physical_or_threat: :physical)

      expect(concern).to be_physical
    end

    it "returns false if the concern does not have a physical component" do
      concern = build(:safety_concern, category: :assaultive_law, physical_or_threat: :threat)
